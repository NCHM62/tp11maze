...
// ReprÃ©sente un chemin sous la forme d'une chaÃ®ne de caractÃ¨res
// composÃ©e des lettres u (up), d (down), l (left), r (right).
typedef char *path_t;
...
typedef struct
{
  ...
  int nbDirections; // ou nbrVoisins : c'est Ã  dire le nb de cases utiles dans directionsPossibles et QuatreVoisins
...
typedef struct
{
  ...
  // |  _|   | | |
  // |_|  _| | | row(6)
  // | __|___  | |es politiques, syndicaux et Ã©tudiants a appelÃ© Ã  une large manifestation Ã  Ajaccio, dimanche 3 avril. Une dÃ©cision qui n'a pas remportÃ© les faveurs de l'ensemble des membres du collecti
  // |_______|v| v
  // <--col(5)->
  ...
  // L'entrÃ©e se trouve dans la cellule supÃ©rieure gauche
  // (nord-ouest), la sortie se trouve dans la cellule infÃ©rieure
  // droite (sud-est).

  // wallh[i][j] et wallv[i][j] encodent pour la cellule (i,j) le fait
  // qu'elle comporte un mur horizontal au dessous ou vertical Ã 
  ...
  int col;
  int num;
  // table de boolÃ©ens murs horizontaux '_'
  bool *wallh;
  // table de boolÃ©ens murs verticaux '|'
  bool *wallv;
  T_cellule *cells;
...
{ /////////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\MAIN
  ...
  /* maze_t *m = init_maze(6, 5);
   if (!m) {
     printf("Echec d'allocation mÃ©moire pour le labyrinthe.\n");
     return 0;
   }
...
// AccÃ¨s (en lecture et Ã©criture) au mur vertical Ã  droite de la
...
// AccÃ¨s (en lecture et Ã©criture) au mur horizontal au dessous de la
// cellule row, col du labyrinthe m
bool *wall_under(maze_t *m, int row, int col)
{
...
// Retourne un pointeur sur un maze_t complÃ¨tement initialisÃ© ou NULL
// si l'allocation a Ã©chouÃ©.
maze_t *init_maze(int row, int col)
{
  maze_t *m;
...
// Â« Labyrinthe dans lequel toutes les cellules sont fermÃ©es.
// Exemple : Le Â« labyrinthe Â» compartimentÃ© 6 lignes, 5 colonnes
// ._________.
// |_|_|_|_|_|
// |_|_|_|_|_|
// |_|_|_|_|_|
...
maze_t *compartmentalized_maze(int row, int col)
{
  ...
  // DerniÃ¨re colonne (murs horizontaux) :
  for (int i = 0; i < m->row - 1; ++i)
    *wall_under(m, i, m->col - 1) = 1;
  // DerniÃ¨re ligne (murs verticaux) :
  for (int j = 0; j < m->col - 1; ++j)
...
// Les caractÃ¨res utilisÃ©s pour l'affichage sont
// '_' pour un mur en dessous de la cellule considÃ©rÃ©e,
// '|' pour un mur Ã  droite de la cellule considÃ©rÃ©e.
// ----------------------------------------------------------------------------------------------------------------
void print_maze(maze_t const *m, char *udlr, int t[200][2], int n)
{
  print_maze_raw(m);
